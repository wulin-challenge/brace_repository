...
public abstract class ScriptUtils {
	 ...
	 */
	public static void executeSqlScript(Connection connection, String script,SingleSql singleSql) throws ScriptException {
	...
	public static void executeSqlScript(Connection connection, String script, boolean continueOnError,
			...
			String blockCommentStartDelimiter, String blockCommentEndDelimiter,SingleSql singleSql) throws ScriptException {

		executeSqlScript(connection, script, continueOnError, ignoreFailedDrops,
				new String[] { commentPrefix }, separator, blockCommentStartDelimiter,
				blockCommentEndDelimiter,singleSql);
	}
	...
	public static void executeSqlScript(Connection connection, String script, boolean continueOnError,
		...
		try {
			if (logger.isDebugEnabled()) {
				logger.debug("Executing SQL script from " + script);
			}
			...
			if (!EOF_STATEMENT_SEPARATOR.equals(separator) &&
					!containsStatementSeparator(script, separator, commentPrefixes,
						blockCommentStartDelimiter, blockCommentEndDelimiter)) {
				separator = FALLBACK_STATEMENT_SEPARATOR;
			}

			List<String> statements = new ArrayList<>();
			splitSqlScript(script, separator, commentPrefixes, blockCommentStartDelimiter,
			...
			try {
				for (String statement : statements) {
					...
					try {
						//执行SQL回调
						if(singleSql != null) {
							statement = singleSql.sql(statement);
						}
						...
						if (continueOnError || (dropStatement && ignoreFailedDrops)) {
							if (logger.isDebugEnabled()) {
								logger.debug(ScriptStatementFailedException.buildErrorMessage(statement, stmtNumber, null), ex);
							}
						}
						else {
							throw new ScriptStatementFailedException(statement, stmtNumber, null, ex);
						}
					}
				}
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Executed SQL script from sqlscript in " + elapsedTime + " ms.");
			}
		}
		catch (Exception ex) {
			...
			throw new UncategorizedScriptException(
				"Failed to execute database script from resource [sqlscript]", ex);
		}
	}
	...
	public static boolean containsSqlScriptDelimiters(String script, String delimiter) {
		return containsStatementSeparator(script, delimiter, DEFAULT_COMMENT_PREFIXES,
	 ...
	 * @since 5.2.16
	 */
	private static boolean containsStatementSeparator(String script,
	...
	public static void splitSqlScript(@Nullable EncodedResource resource, String script,
		...
		splitSqlScript(script, separator, new String[] { commentPrefix },
	 ...
	 *
	 */
	public static interface SingleSql{
